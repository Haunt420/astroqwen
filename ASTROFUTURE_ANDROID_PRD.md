...
Build an original Android astrology app that matches the observable function, information architecture, interaction patterns, and polish of Astro Future as closely as is practically and legally permissible, while avoiding direct copying of brand assets, copyrighted text, screenshots, exact layouts, and proprietary implementation details.

The product should feel like a native Android counterpart to Astro Future:
...
- monetized with premium unlocks
...
- Include widgets and premium features.
...
- Copy Astro Future branding, iconography, wording, screenshots, or artwork.
- Reproduce exact UI composition pixel-for-pixel.
- Reuse proprietary content or private data structures.
- Present the app as affiliated with Astro Future.

### Assumptions

- The app is astrology entertainment software, not a medical, financial, or predictive decision engine.
- Users will often arrive to inspect daily guidance, compare compatibility, and browse charts casually.
- The best Android analogs are Material 3, bottom navigation, adaptive layouts, widgets, and optional Wear OS.

## Product Principles

1. **Chart first**: the wheel or chart view is the anchor, not an afterthought.
...
6. **Premium without friction**: premium unlocks should be legible and easy to restore.
7. **Android-native behavior**: adaptive layouts, back handling, widget support, and shares should feel standard on Android.
...
| Daily horoscope in app and on widgets | Home daily reading, widget tiles, and notifications |
...
| Premium time extension | Extended transit range, gated by entitlement |
...
| Apple Watch support | Optional Wear OS surface and/or widget-first equivalent |
...
- The home screen should also work as a widget target and as a compact tablet pane.
...
- Optional premium badge if the interpretation is locked
...
- Premium gating, if any, is transparent before the user reaches the locked content.
...
- Produce a social-card style image for a chart, reading, or compatibility result.
...
- Users can export a composited image of the current reading or chart.
- The image respects the current theme and selected profile.
- The result is legible on social platforms and messaging apps.
...
- Premium gating, if used, is explicit.
...
- Keep the app present throughout the day on the home screen.
...
- Widgets render correctly in at least small, medium, and large sizes.
- Widget taps deep-link into the right app screen.
- Widget content respects the selected profile or default profile.

### 18. Notifications

**Purpose**

- Re-engage users with meaningful, non-spammy reminders.
...
- Transit event
- Special event
- Subscription reminder, where permitted
...
- Convert engaged users without making the free experience unusable.

**Premium Unlocks**

- Unlimited compatibility horoscopes
- Extended time range
- Detailed transit interpretations
- Special event interpretations
- Alternative house systems
- Additional reports or export options, if desired

**Behavior**

- Present a clear free-versus-premium split.
- Explain the value of each premium capability before purchase.

**Acceptance Criteria**

- The paywall accurately reflects the current entitlements.
- Purchases can be restored.
- Locked premium content is not silently exposed.
...
- Users can manage subscriptions and restore purchases.
...
- Free users should still get a satisfying core experience.
- Premium content should appear locked, not broken.
...
- **Responsive design**: single-pane phone, two-pane tablet/foldable.
- **Reliability**: chart generation should be deterministic for the same inputs and settings.
- **Privacy**: birth data should be treated as sensitive user data.
...
- widgets and notifications provide daily utility
- premium unlocks map cleanly to the reference’s paid capabilities
- the product feels like an Android-native answer to Astro Future without copying its identity

## Open Decisions

- Whether compatibility gets a numerical score, a qualitative score, or both
- Whether profile data syncs cross-device or stays local-only
- Whether Wear OS is shipped in v1 or deferred
- Whether the free tier includes sample charts for unauthenticated browsing
- Whether astrology interpretation content is bundled offline or fetched dynamically
...
- PDF reports and shareable snapshots
- elements, qualities, and dualities
- premium time extension from 1900 to 2050
- detailed transit interpretations
- special events such as eclipses, solstices, equinoxes, retrogrades, and planetary birthdays
- selectable house systems

This PRD preserves those observable functions while keeping implementation and presentation original.
